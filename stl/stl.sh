...
tunnel() {
	...
	# Baca mode saat ini untuk memastikan logic
	cur_mode="$(cat /root/akun/pillstl.txt)"
	...
	for i in {1..3}; do
		...
		if [ "$var" = "SUCCESSFULLY" ]; then
			# MODIFIKASI: Cek Mode
			if [ "$cur_mode" = "3" ]; then
				echo "Socks Only Connected (Port 1080) - Ready for OpenClash"
			else
				gproxy
				echo "Connected"
			fi
...
start() {
	...
	if [[ $pillstl = "1" ]]; then
		...
		# Mode 3: Socks Only (Tanpa routing, Tanpa iptables)
		echo "Starting Socks Only Mode..."
		tunnel
	fi
...
stop() {
	...
	if [[ $pillstl = "1" ]]; then
		...
		# Stop Mode 3
		killall -q python3 ssh sshpass autorekonek-stl
		echo "Socks Only Stopped."
	fi
...
if [ "$tools" = "1" ]; then
	...
	echo "3. Socks only (clash needed)"
...
"
		...
		# Dummy badvpn command agar tidak error jika terpanggil (safety)
		badvpn="echo 'Socks Only Mode Active - No Routing Applied'"
	else
